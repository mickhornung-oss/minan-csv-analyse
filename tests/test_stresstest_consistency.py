...
class TestStressTestConsistency:
    ...
    def test_stresstest_spaltenanzahl_konsistent(self, stresstest_data):
        ...
        assert profile.column_count == 21, f"Profile hat {profile.column_count} Spalten, erwartet 21"
    ...
    def test_stresstest_zeilenanzahl_konsistent(self, stresstest_data):
        ...
        assert profile.row_count == 5000, f"Profile hat {profile.row_count} Zeilen, erwartet 5000"
    ...
    def test_stresstest_spaltentypen_korrekt(self, stresstest_data):
        ...
        assert profile.numeric_columns == 5, f"Expected 5 numeric, got {profile.numeric_columns}"

        # Sollte mindestens 6 kategorische haben
        assert profile.categorical_columns >= 6, f"Expected 6+ categorical, got {profile.categorical_columns}"

        # Sollte 2 Datetime haben
        assert profile.datetime_columns == 2, f"Expected 2 datetime, got {profile.datetime_columns}"
    ...
    def test_stresstest_fehlwerte_in_quality(self, stresstest_data):
        ...
        assert any(name in high_missing_names for name in ['Kundennote', 'Zweites_Produkt']), \
    ...
    def test_stresstest_summary_erwähnt_spaltenanzahl(self, stresstest_data):
        ...
        assert "21" in summary_text or "spalten" in summary_text, \
    ...
    def test_stresstest_summary_erwähnt_zeilenanzahl(self, stresstest_data):
        ...
        assert "5000" in summary_text or "zeilen" in summary_text, \
    ...
    def test_stresstest_quality_findings_priorisiert(self, stresstest_data):
        ...
        if len(quality.findings) > 0:
            ...
            critical_indices = [i for i, s in enumerate(first_severities) if s == "critical"]
            warning_indices = [i for i, s in enumerate(first_severities) if s == "warning"]
    ...
    def test_stresstest_duplikate_keine(self, stresstest_data):
        ...
        assert quality.duplicate_rows == 0, f"Expected 0 duplicate_rows, got {quality.duplicate_rows}"
    ...
    def test_stresstest_id_spalten_erkannt(self, stresstest_data):
        ...
        assert 'KundenNummer' in id_names or 'Bestellnummer' in id_names, \
    ...
    def test_stresstest_summary_keine_duplikate_erwähnung(self, stresstest_data):
        ...
        if quality.duplicate_rows == 0:
            ...
            if "duplikat" in summary_text:
                assert "keine" in summary_text, "Sollte 'keine' sagen wenn keine Duplikate"
